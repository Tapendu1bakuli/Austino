...
class TabBarWidget extends StatelessWidget implements PreferredSize {
  ...
  Widget buildTabBar() {
    ...
    if (controller.selectedId?.firstRebuild ?? false) {
      ...
      return Container(
        ...
        child: RawChip(
          ...
          shape: OutlinedBorder.lerp(
              RoundedRectangleBorder(side: BorderSide(color: AppColors.white)),
              RoundedRectangleBorder(side: BorderSide(color: AppColors.white)),
              0.0),
          label: Text(text), focusNode: FocusNode(),
          labelStyle: controller.isSelected(this.id)
              ? _style?.merge(TextStyle(color: Colors.black))
              : _style,
          padding: _padding,
